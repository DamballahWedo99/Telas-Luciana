...
import { Loader2, TruckIcon, CalendarIcon, ContainerIcon, UserIcon, NotebookPenIcon, Search, EyeIcon, EditIcon, TrashIcon } from "lucide-react";
...
export const LogisticsModal: React.FC<LogisticsModalProps> = ({
  ...
  const [deleteConfirmation, setDeleteConfirmation] = useState<LogisticsData | null>(null);
  ...
  const filteredLogisticsOrders = React.useMemo(() => {
    ...
    return logisticsOrders.filter(order =>
      ...
      order.tela?.some(tela => tela.tipo_tela.toLowerCase().includes(searchQuery.toLowerCase()))
    );
  ...
  const confirmDeleteOrder = async () => {
    ...
    try {
      const response = await fetch(`/api/s3/logistica?orden_de_compra=${encodeURIComponent(deleteConfirmation.orden_de_compra)}`, {
        method: "DELETE",
      });

      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || "Error al eliminar la información logística");
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const requestBody = isEditing && originalOrderName !== formData.orden_de_compra
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || `Error al ${isEditing ? 'actualizar' : 'guardar'} la información logística`);
      }

      const result = await response.json();

      toast.success(`Información logística ${isEditing ? 'actualizada' : 'guardada'} exitosamente`, {
      ...
      console.log(`✅ Información logística ${isEditing ? 'actualizada' : 'guardada'}:`, result);
    } catch (error) {
      console.error(`❌ Error ${isEditing ? 'actualizando' : 'guardando'} información logística:`, error);
      
      toast.error(`Error al ${isEditing ? 'actualizar' : 'guardar'} la información logística`, {
        description: error instanceof Error ? error.message : "Error desconocido",
        duration: 5000,
      });
    } finally {
          ...
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              {viewDetails 
                ...
                : showForm 
                  ? (editingOrder ? "Editar Información Logística" : "Nueva Información Logística")
              ...
              <div className="grid gap-4">
                <div className="border rounded-lg p-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <Label className="text-sm font-medium text-gray-700">Orden de Compra</Label>
                      <p className="mt-1 text-sm bg-gray-50 p-2 rounded font-mono">{viewDetails.orden_de_compra}</p>
                    </div>
                    <div>
                      <Label className="text-sm font-medium text-gray-700">Contenedor</Label>
                      <p className="mt-1 text-sm bg-gray-50 p-2 rounded">{viewDetails.contenedor || "No especificado"}</p>
                    </div>
                    <div>
                      <Label className="text-sm font-medium text-gray-700">Importador</Label>
                      <p className="mt-1 text-sm bg-gray-50 p-2 rounded">{viewDetails.importador || "No especificado"}</p>
                    </div>
                    <div>
                      ...
                      <p className="mt-1 text-sm bg-gray-50 p-2 rounded">
                        {viewDetails.fecha_registro 
                          ? new Date(viewDetails.fecha_registro).toLocaleDateString('es-ES', {
                              year: 'numeric',
                              month: 'long',
                              day: 'numeric',
                              hour: '2-digit',
                ...
                <div className="border rounded-lg p-4">
                  <h3 className="font-semibold text-lg mb-4">Fechas de Envío</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <Label className="text-sm font-medium text-gray-700">ETD (Fecha de Salida)</Label>
                      <p className="mt-1 text-sm bg-gray-50 p-2 rounded">
                        {viewDetails.etd ? new Date(viewDetails.etd).toLocaleDateString('es-ES') : "No especificado"}
                      </p>
                    </div>
                    <div>
                      <Label className="text-sm font-medium text-gray-700">ETA (Fecha de Llegada)</Label>
                      <p className="mt-1 text-sm bg-gray-50 p-2 rounded">
                        {viewDetails.eta ? new Date(viewDetails.eta).toLocaleDateString('es-ES') : "No especificado"}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                {viewDetails.tela && viewDetails.tela.length > 0 && (
                  <div className="border rounded-lg p-4">
                    <h3 className="font-semibold text-lg mb-4">Telas ({viewDetails.tela.length})</h3>
                ...
                {viewDetails.notas && (
                  <div className="border rounded-lg p-4">
                    ...
                    <p className="text-sm bg-gray-50 p-3 rounded whitespace-pre-wrap">{viewDetails.notas}</p>
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="grid gap-3 max-h-[350px] overflow-y-auto">
                {isLoadingLogistics ? (
                  ...
                  <Card>
                    <CardContent className="flex items-center justify-center py-8">
                      <div className="text-center text-muted-foreground">
                        ...
                        <p>{searchQuery.trim() ? "No se encontraron órdenes que coincidan con la búsqueda" : "No hay órdenes de logística disponibles"}</p>
                      </div>
                    </CardContent>
                  </Card>
                ) : (
                  filteredLogisticsOrders.map((logisticsOrder) => {
                    return (
                      <Card key={logisticsOrder.orden_de_compra} className="hover:shadow-md transition-shadow">
                        <CardContent className="p-4">
                          <div className="flex items-center justify-between">
                            <span className="font-mono text-lg font-semibold">{logisticsOrder.orden_de_compra}</span>
                              ...
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <Button
                                    variant="outline"
                                    size="sm"
                                    onClick={() => handleViewDetails(logisticsOrder)}
                              ...
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <Button
                                    variant="outline"
                                    size="sm"
                                    onClick={() => handleDeleteOrder(logisticsOrder)}
                                    disabled={isDeleting}
            ...
            <div className="space-y-6 py-4">
              ...
              <div className="space-y-2">
                ...
                {formErrors.orden_de_compra && (
                  <p className="text-sm text-red-600">{formErrors.orden_de_compra}</p>
                )}
              </div>
                  ...
                  <div className="space-y-3">
                    {formData.tela.map((tela, index) => (
                      <div key={index} className="border rounded-lg p-4 space-y-3">
                        <div className="flex items-center justify-between">
                          <span className="text-sm font-medium text-gray-700">Tela {index + 1}</span>
                          <Button
                            type="button"
                            variant="ghost"
                            size="sm"
                        ...
                        <div className="space-y-2">
                          <Label htmlFor={`tela-${index}-tipo`}>Tipo de Tela</Label>
                          <Input
                            id={`tela-${index}-tipo`}
                            type="text"
                            ...
                            onChange={(e) => {
                              ...
                              setFormData(prev => ({ ...prev, tela: newTelas }));
                            }}
                          ...
                          {formErrors[`tela.${index}.tipo_tela`] && (
                            <p className="text-sm text-red-600">{formErrors[`tela.${index}.tipo_tela`]}</p>
                          )}
                        </div>
                      </div>
                    ))}
      ...
      <Dialog open={!!deleteConfirmation} onOpenChange={() => setDeleteConfirmation(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Esta acción no se puede deshacer. Se eliminará permanentemente toda la información logística.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="py-4">
            <div className="bg-red-50 border border-red-200 rounded-lg p-3 mb-4">
              <p className="text-sm text-red-800">
                <strong>Orden de compra:</strong> <span className="font-mono">{deleteConfirmation?.orden_de_compra}</span>
              </p>
              ...
              {deleteConfirmation?.tela && deleteConfirmation.tela.length > 0 && (
                <p className="text-sm text-red-800 mt-1">
                  <strong>Telas registradas:</strong> {deleteConfirmation.tela.length}
                </p>
              )}
            </div>
