...
interface LoteComboboxProps {
  ...
  lotesWithModifications?: Set<string>;
}
...
export function LoteCombobox({
  ...
  disabled = false,
  rollCounts = {},
  lotesWithModifications = new Set(),
}: LoteComboboxProps) {
  const [open, setOpen] = React.useState(false);
  const [search, setSearch] = React.useState("");
  const containerRef = React.useRef<HTMLDivElement>(null);

  // Create options without "all" option
  const options = React.useMemo(() => {
    return lotes.map(lote => ({
      value: lote,
      label: lote,
      count: rollCounts[lote] || 0
    }));
  }, [lotes, rollCounts]);
        ...
        <span className="truncate">
          {selectedOption ? (
            <div className="flex items-center space-x-2">
              ...
              {lotesWithModifications.has(selectedOption.value) && (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between w-full">
                      <div className="flex items-center space-x-2">
                        ...
                        {lotesWithModifications.has(option.value) && (
                          <div className="w-2 h-2 bg-orange-500 rounded-full" title="Tiene modificaciones pendientes" />
                        )}
                      </div>
                      <span className="text-sm text-gray-500">
